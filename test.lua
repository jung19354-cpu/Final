-- ==========================================
-- DOORS AUTO-PLAY V4 (AUF ECHTEN SPIEL-DATEN)
-- BASIERT AUF AGENT-RECHERCHE
-- ==========================================

print("🚀 Lade Doors Auto-Play V4 (Agent-basiert)...")
...
-- ==========================================
-- SPIEL-STATE VARIABLEN (aus Agent-Bericht)
-- ==========================================
...
-- Knobs im aktuellen Raum finden
...
-- Gegenstand aufheben (via ProximityPrompt)
...
-- ==========================================
-- AUTO-PLAY HAUPTLOGIK (basierend auf Agent-Bericht)
-- ==========================================
...
local function startAutoPlay()
    ...
    print("🔄 Auto-Play V4 gestartet!")
    print("📌 Warte auf Raum-Änderungen...")
    
    -- Listener für Raum-Änderungen (aus Agent-Bericht)
    local function onRoomChanged()
        ...
        print("🏠 Raum " .. roomNum .. " erreicht!")
        
        -- Den Raum-Container holen
        local room = getCurrentRoomContainer()
        if not room then
        ...
        if getgenv().AutoCollect then
            ...
            for _, key in pairs(keys) do
                local keyPos = key:IsA("BasePart") and key.Position or (key:FindFirstChild("Handle") and key.Handle.Position)
                if keyPos then
                    teleportTo(keyPos + Vector3.new(0, 1, 0))
                    wait(0.15)
            ...
            for _, knob in pairs(knobs) do
                local knobPos = knob:IsA("BasePart") and knob.Position or (knob:FindFirstChild("Handle") and knob.Handle.Position)
                if knobPos then
                    teleportTo(knobPos + Vector3.new(0, 1, 0))
                    wait(0.15)
        ...
        -- ==========================================
        -- 2. AUTO-OPEN-DOORS (mit State-Polling)
        -- ==========================================
        if getgenv().AutoOpenDoors or getgenv().AutoRooms then
            local door = findDoorInRoom(room)
            if door then
                -- Versuche Tür zu öffnen (durch Interaktion simulieren)
                local handle = door:FindFirstChild("Handle")
                if handle then
                    local prompt = handle:FindFirstChild("ProximityPrompt")
                    if prompt then
                        prompt:FireServer()
                        print("🚪 Tür-Interaktion gesendet...")
                    end
                end
                
                -- Warten, bis Tür geöffnet ist (State-Polling aus Agent-Bericht)
                print("⏳ Warte auf Tür-Öffnung...")
                local timeout = 30 -- 30 Sekunden Timeout
                ...
                -- Falls geöffnet, durch die Tür gehen
                if isDoorOpen(door) then
                    local doorPos = handle and handle.Position or door.PrimaryPart and door.PrimaryPart.Position
                    if doorPos then
                        teleportTo(doorPos + Vector3.new(0, 0, 3))
                        wait(0.3)
                    end
                end
            else
                print("   ❌ Keine Tür in diesem Raum gefunden!")
            end
        end
        
        -- ==========================================
        -- 3. AUTO-ROOMS (A-1000 Farming)
        -- ==========================================
        if getgenv().AutoRooms then
            -- Spezielle Rooms-Logik (wenn im Rooms-Modus)
            for _, obj in pairs(room:GetDescendants()) do
                if obj.Name == "RoomDoor" or obj.Name == "ExitDoor" then
    ...
    -- Raum-Änderungen überwachen (aus Agent-Bericht)
    local connection
    ...
    -- Initiale Raum-Änderung auslösen
    wait(0.5)
    onRoomChanged()
end
...
screenGui.Name = "DoorsAutoPlayV4"
...
createButton("📦 Aktuellen Raum scannen", Color3.fromRGB(130, 130, 50), function()
    local roomNum = getCurrentRoom()
    local room = getCurrentRoomContainer()
    if room then
        print("🔍 Raum " .. roomNum .. " gescannt!")
        print("   - Türen:", findDoorInRoom(room) and "✅" or "❌")
        print("   - Schlüssel:", #findKeysInRoom(room))
        print("   - Knobs:", #findKnobsInRoom(room))
    end
end)
...
print("✅ Doors Auto-Play V4 geladen!")
print("📌 Basierend auf Agent-Recherche!")
print("📌 Aktiviere 'Auto-Play' für Vollautomatik!")
print("📌 Das Skript reagiert auf Raum-Änderungen!")-- ==========================================
