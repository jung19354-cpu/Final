...
local ok, errMsg = pcall(function()
    ...
    local function isOn(btn) 
        return btn.BackgroundColor3 == Color3.new(0.1, 0.45, 0.25) 
    end
    ...
    spawn(function() 
        while true do 
            ...
            if not isOn(tESP) then
                ...
                -- continue nicht möglich, also einfach weiter
            else
                for _, f in pairs(findFruits()) do
        ...
        while true do 
            ...
            if isOn(tQuest) then
                ...
                if bestQ then
                    ...
                    if npc then
                        -- Korrektur: npc.Position ist nicht direkt verfügbar, verwende npc:GetPivot() oder PrimaryPart
                        local npcPos = npc:GetPivot()
                        if npcPos then
                            teleport(npcPos.Position + Vector3.new(0, 10, 0))
                            wait(1)
                            local ok = pcall(function()
    ...
    spawn(function() 
        while true do 
            ...
            info = info .. "\nLevel: " .. playerLevel()
            Status.Text = info -- Status.Text immer überschreiben
        end 
    end)
